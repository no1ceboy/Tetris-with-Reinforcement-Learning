...
class Tetris:
    ...
    def reset(self):
        ...
        self._new_round()
        self.score = 0
        return np.asarray(self.board, np.float32)
        ...
        for rotation in rotations:
            ...
            for x in range(-min_x, Tetris.BOARD_WIDTH - max_x):
                ...
                if pos[1] >= 0:
                    ...
                    states[(x, rotation)] = np.asarray(board, np.float32)
    ...
    def play(self, x, rotation, render=False, render_delay=None):
        ...
        if not self.game_over:  
            ...
            score = 1 + (lines_cleared ** 2) * Tetris.BOARD_WIDTH / 2 - 0.3 * holes - 0.05 * total_bumpiness - 0.03 * sum_height
        ...
        return score, self.game_over, lines_cleared, np.asarray(self.board, np.float32)
